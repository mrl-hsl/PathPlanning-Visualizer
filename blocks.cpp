...
struct initRoi {
  ...
  int grabType; // it's for when you drag a black block, you don't make any blalck blocks white or the other turn around
} SelectedRoi;

// global variables
const int pixel = 50; // the size of each block generated 
...
void ChangeColor(Mat image, vector<vector<int>>& vec, int x, int y);
void MakeBlocks(Mat image, vector<vector<int>>& vec);

// main function
int main() {
    SelectedRoi.init = 0; // SelectedRoi.init is zero by default
    system("clear"); // to clear-out the previous texts shown on the terminal
    ...
    vec.resize(H_ratio, vector<int>(V_ratio, 0));
...
void CallBackFunc(int event, int x, int y, int flags, void* userdata) {
    ...
    if (event == EVENT_LBUTTONUP) {
        SelectedRoi.init = 0; // button is now down anymore (value == 0)
        return;
    }

    // making two new values to use later on
    int changeY = abs((int)(SelectedRoi.initX / pixel) - (X)); // knowing the block change in the x axis
    int changeX = abs((int)(SelectedRoi.initY / pixel) - (Y)); // knowing the block change in the y axis
    
    // if the left button is still down:
        ...
        if(event == EVENT_MOUSEMOVE) {
            ...
            if(changeX >= 1 || changeY >= 1) {
                ...
                if(SelectedRoi.grabType != vec[X][Y]) {
                    ...
                    // calls the function that changes the color of the block
                    ChangeColor(image, vec, x, y);
...
void ChangeColor(Mat image, vector<vector<int>>& vec, int x, int y) {
    ...
    if(vec[X][Y] == 0) {
        ...
        vec[X][Y] = 1; // reseting the value of the block, because is has been changed
    }
    ...
    // if the block is black it changes to white
        ...
        vec[X][Y] = 0; // reseting the value of the block, because is has been changed
    }
    imshow("Output", image);
}